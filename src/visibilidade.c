...
int cmp_eventos(const void* a, const void* b) {
    ...
    if (ea->angulo > eb->angulo) return 1;
    if (ea->tipo == 0 && eb->tipo == 1) {
        return -1;
    } else if (ea->tipo == 1 && eb->tipo == 0) {
        return 1;
    }
...
int cmp_segmentos_ativos(void* a, void* b) {
    ...
    if (fabs(distA - distB) < 1e-5) {
        if (get_ant_id(antA) < get_ant_id(antB)) {
            return -1;
        } else {
            return 1;
        }
...
Poligono calcular_visibilidade(float x_bomba, float y_bomba, LISTA lista_anteparos) {
    ...
    insere_vertice(pol_vis, x_bomba, y_bomba); 

    int cap_eventos = 100; 
    int tam_lista = tamanho_lista(lista_anteparos);
    ...
    POSIC no = get_primeiro_no(lista_anteparos);
    
    while (no != NULL) {
        Anteparo ant = (Anteparo) get_info_do_no(lista_anteparos, no);
        ...
        no = get_proximo_no(lista_anteparos, no);
    }
    ...
    libera_arvore(segmentos_ativos);
    free(eventos);
